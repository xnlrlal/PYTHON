...
print("----------------------------------------------------------------------")
# 각 정보별 특징 자세히 출력하기 (샘플 수, 평균, 표준편차, 최소값, 백분위 수로 25%, 50%, 75%에 해당하는 값, 최대값)
print(df.describe())

print("----------------------------------------------------------------------")
# 각 항목 어느 정도의 상관 관계 확인
print(df.corr())
...
# plasma를 기준으로 각각 정상과 당뇨가 어느 정도 비율로 분포하는지 확인하기
'''
    hist() 함수 안에 x축 지정 (가져올 컬럼)
        - diabetes 값이 0인 것과 1인 것을 구분해 불러옴 
        - bins : x축을 몇 개의 막대로 쪼개어 줄 것인지 정함
    => plasma 수치가 높아질수록 당뇨인 경우가 많음  
'''
plt.hist(x=[df.plasma[df.diabetes == 0], df.plasma[df.diabetes == 1]]
         , bins=30, histtype='barstacked', label=['noramal', 'diabetes'])
plt.legend()
plt.show()
...
'''
    ...
    평균이나 중앙값으로 대치하거나, 이상치를 제거하는 과정 등이 데이터 전처리에 포함 가능 (성능 높임)
'''
