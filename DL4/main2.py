...
# astype() 메서드를 이용하여 데이터를 범주형(단어)으로 변환
...
outputs = pd.get_dummies(dataset.output)  # get_dummies() : 가변수(dummy variable)로 만들어 주는 함수 (0, 1)
...
print("-" * 70)
total_records = 1728
test_records = int(total_records * .2)  # 데이터 셋을 훈련 셋과 테스트 셋 용도로 분리
...
# 모델의 네트워크 생성
class Model(nn.Module):  # 클래스 형태로 구현되는 모델은 nn.Module을 상속 받음
    '''
        self : 첫 번째 파라미터로 self 지정해야 함. 자기 자신을 가리킴
        embedding_size : 범주형 컬럼의 임베딩 크기
        output_size : 출력 층의 크기
        layers : 모든 계층에 대한 목록
        p : 드롭아웃 (기본값은 0.5)
    '''
...
'''
    ...
    은닉층의 뉴런을 정의는 여러 크기로 지정하여 테스트해 보는 것이 학습에 도움이 될 것임.
'''
...
'''
    ...
    이 DL4 경우 데이터를 분류해야 하는 것으로 크로스 엔트로피(cross entropy) 손실 함수를 사용함 
    옵티마이저는 아담(Adam)을 사용함 
    
    lr(Learning Rate) : 미분값을 얼마만큼 이동시킬 것인가를 결정함.
                        초기값이 크다면 초반에 loss값이 빠르게 줄어들 것임. 나중에 가면 underfitting이 발생함 
'''
print("-"* 80)
...
train_outputs.to(device=device, dtype=torch.int64)
...
print(f"Loss : {loss:.8f}")         # 테스트 데이터셋에 대한 손실값 출력
                                    # 훈련 데이터셋에서 도출된 값과 비슷하므로 과적합은 발생하지 않음
...
# 신경망에서 필요한 모든 파라미터를 무작위로 선택했다고 감안할 때 74%의 정확도는 나쁘지 않음
# 파라미터(훈련/테스트 데이터셋 분할, 은닉층 개수 및 크기 등) 변경하면서 더 나은 성능을 찾아보는 것도 의미가 있을 것임.