'''
    1. 인공 신경망
        ...
        4) 활성화 함수 (activation function)
            ...
            - 입력값과 가중치를 곱한 값들은 황성화 함수를 거쳐 출력값이 됨
            - 시그모이드 함수롸 ReLU 함수
                - sigmoid function
        ...
        5) 경사 하강법
            - 신경망 훈련의 목표는 최적 파라미터를 찾는 것임
                - 파라미터는 가중치와 편향
            - 최적 파라미터는 손실 함수가 최소값일 때의 파라미터를 뜻함.
                - 매 훈력 단계마다 손실값이 줄어드는 방향으로 파라미터를 갱신하며
                  최적의 파라미터를 찾음

            - 손실 함수 (loss function)
                - 모델 성능이 얼마나 좋은지 나쁜지 측정하는 함수
                - 예측한 값과 실제 값의 차이(손실)를 구하는 함수
                    - 당연히 차이는 작을수록 좋음
                    - 손실 함수의 값이 작을수록 좋은 모델이라고 볼 수 있음
                - 평균 제곱 오차(MSE), 교차엔트로피(crossentropy)
                - 손실 함수 값이 최소가 되는 가중치와 편향을 구하는 작업 (신경망 훈련)
            ...
            - 경사 하강법 (gradient descent)
                ...
                - 절차(①②③)
                    ...
                    - 학습률(learning rage)
                        ...
                        - 학습률에 따라 훈련 속도가 달라짐
                            - 한 걸음에 얼마만큼 이동하지 결정하는 게 학습률임
                                - 학습률이 너무 크면 훈련 속도는 빠르지만, 더 빠른 길로 이어지는 지점을 건너뛰어서
                                  최적 가중치를 찾지 못 할 수 도 있음.
        ...
        6) 순전파 & 역전파
            - forward propagation
                ...
                - 각 층을 통과할 때마다 입력값에 가중치를 곱해 다음 층으로 출력할 값을 계산
            - back propagation
                - 순전파의 반대 개념
                - 신경망 훈련에서 가장 중요한 내용임
            ...
            - 과정
                ...
                - 기울기를 바탕으로 가중치를 갱신함. 이때 츌력층 => 입력층 방향으로 가중치를 갱신(역전파)
    ...
    2. 딥러닝 알고리즘 성능 향상
        1) 드롭아웃(dropout)
            - 과대 적합을 방지하기 위하여 신경망 훈련 과정에서 무작위로 일부 뉴런을 제외하는 기법
            - 얼마나 많은 뉴런을 드롭아웃할지는 하이퍼 파라미터로 설정할 수 있음
