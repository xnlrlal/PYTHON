'''
    ...
    3. 벡터
        ...
        2) [1.0, 1.1, 1.2]처럼 숫자들의 리스트, 1처원 배열 형태

       행렬(matrix)
        1) 행과 열로 표형되는 2차원 배열 형태
       ...
       텐서
        1) 3차원 이상의 배열 형태
            ...
            - 3차원의 축(채널) : axis=2 (텐서)
        2) torch.tensor()
        3) 텐서 간 연산을 수행할 때, 기본적으로 두 텐서가 같은 장치에 있어야 함
            - 서로 다른 장치(device)에 있는 텐서끼리 연산을 수행하면 오류가 발생함.

    4. 파이토치 특징
        1) 신경망은 연산 그래프를 이용하여 계산을 수행
        2) 네트워크가 학습될 때 손실 함수의 기울기가 가중치와 바이어스(절편)를 기반으로 계산되며
           이후 경사 하강법의 사용하여 가중치가 업데이트
